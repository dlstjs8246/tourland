...
(1, "가입 축하 할인", "2020-04-27", "2200-12-31", "투어랜드 첫 가입 축하 20% 할인 쿠폰", 20.0), 
(2, "모든 해외여행 총집합 할인", "2020-04-27", "2020-05-04", "모든 해외여행 30% 할인", 30.0),
(3, "여름 휴가 시작 할인", "2020-07-01", "2020-08-01", "여름 휴가 떠나자 할인 쿠폰", 20.0),
(4, "얼리버드 할인 쿠폰", "2020-06-01", "2020-06-30", "얼리버드를 잡아라 할인 쿠폰", 10.0),
(5, "동남아시아 패키지 할인", "2020-05-01", "2020-05-30", "동남아시아로 떠나는 패키지 할인 쿠폰", 12.0),
(6, "도쿄의 겨울 패키지 할인", "2019-11-11", "2019-12-20", "도쿄의 겨울을 즐기러, 10% 할인 쿠폰", 10.0);
...
delimiter $
create trigger tri_after_insert_user
...
create view totalpricebymonth as select substring(pname,2,3) as 'location',year(rdate) as 'year',month(rdate) as 'month',ifnull(sum(p.pprice),0) as 'totalprice' from reservation r 
join userpstatus u on u.no = r.no and u.userno = r.userno
